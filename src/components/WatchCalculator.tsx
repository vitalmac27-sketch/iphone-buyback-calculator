...
import {
  ...
  watchScreenOptions,
  watchBodyOptions,
  watchCompletenessItems,
  getWatchCompletenessMultiplier,
  getSizeMultiplier,
  type WatchConditionOption,
...
const WatchCalculator = ({ onBack, onRestart }: WatchCalculatorProps) => {
  ...
  const currentIndex = WATCH_STEPS.indexOf(step);
  const totalSteps = WATCH_STEPS.length - 1;
  const progressStep = currentIndex >= 0 && step !== "result" ? currentIndex + 1 : 0;

  const selectedModel = watchModels.find((m) => m.id === selectedModelId);

  const goNext = () => {
    const idx = WATCH_STEPS.indexOf(step);
    if (idx < WATCH_STEPS.length - 1) setStep(WATCH_STEPS[idx + 1]);
  };
  ...
  const toggleCompleteness = (item: WatchCompletenessItem) => {
  ...
  return (
    <>
      ...
      <div className="animate-fade-in-up">
        {step === "model" && (
          <div>
            <StepHeader title="Модель Apple Watch ⌚" subtitle="Какие у вас часы?" />
        ...
        {step === "size" && selectedModel && (
          <div>
            <StepHeader title="Размер корпуса 📐" subtitle="Какой размер у ваших часов?" />
            <div className="space-y-3">
        ...
        {step === "completeness" && (
          <div>
            ...
            <div className="space-y-3">
              {watchCompletenessItems.map((item) => {
        ...
        {step === "result" && (
          <div className="text-center space-y-8">
            ...
            <div ref={contactButtonsRef} className="flex flex-col gap-3">
              ...
              <button onClick={() => copyAndOpen(vkUrl())}
